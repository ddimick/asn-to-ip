...
def whois(_query, _server = 'whois.radb.net', _port = 43, _result = str()):
  _sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
  _sock.connect((_server, _port))
  _sock.send(_query.encode('utf-8'))
...
def get_network_list(_result = list()):

  for _asn in _parser_args.asn:
    # Validate we have a valid-looking ASN.
    ...
    # Perform the whois query for IPv4 networks.
    _asn_networks = list(whois('!g{0}\n'.format(_valid_asn)).split())

    # Perform the whois query for IPv6 networks (if requested).
    if _parser_args.ipv6:
      _asn_networks += list(whois('!6{0}\n'.format(_valid_asn)).split())

    # Builds an object containing valid networks, removing duplicates.
    for _network in set(_asn_networks):
      try:
        _result.append(ipaddress.ip_network(_network, strict = False))
      except ValueError:
        # Ignore anything that isn't a valid network.
        continue
